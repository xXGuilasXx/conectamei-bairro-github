...
const SERVICE_IMG = "https://d2xsxph8kpxj0f.cloudfront.net/310519663511049766/AMmQMALUQtyfHWkNibXH3y/service-provider-7HYnMPWz4kYcqG9vLhHbZZ.webp";
    ...
    <div className="py-10">
      <div className="container">
        ...
        <div className="flex flex-col md:flex-row md:items-end gap-6 mb-10">
          ...
          <img src={SERVICE_IMG} alt="Prestador MEI" className="w-28 h-28 rounded-2xl object-cover shadow-md hidden md:block" />
        </div>
        ...
        <div className="flex flex-wrap gap-4 mb-8 p-4 bg-muted/50 rounded-xl">
          ...
          <div className="flex items-end">
            <Button variant="outline" size="sm" onClick={() => { setCatFiltro("todas"); setBairroFiltro("todos"); }}>
              Limpar filtros
            </Button>
          </div>
        </div>
        ...
        {/* Results */}
        <p className="text-sm text-muted-foreground mb-4">{filtrados.length} prestador(es) encontrado(s)</p>

        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtrados.map((p) => (
            <Card key={p.id} className="group hover:shadow-lg transition-shadow duration-200 overflow-hidden">
              <CardContent className="p-5 space-y-3">
                <div className="flex items-start justify-between">
                  <div>
                    <h3 className="font-bold text-lg leading-tight group-hover:text-primary transition-colors">{p.nome}</h3>
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      {p.disponivel ? (
                        ...
                        <span className="text-xs text-muted-foreground font-medium">Indisponível</span>
                      )}
                    </div>
                  </div>
                ...
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {p.servicos.map((s) => (
                    <span key={s} className="text-[11px] px-2 py-0.5 rounded-full bg-primary/5 text-primary font-medium">{s}</span>
                  ))}
                </div>
        ...
        {filtrados.length === 0 && (
          <div className="text-center py-16">
            ...
            <Button variant="outline" className="mt-4" onClick={() => { setCatFiltro("todas"); setBairroFiltro("todos"); }}>
              Limpar filtros
            </Button>
          </div>
        )}
